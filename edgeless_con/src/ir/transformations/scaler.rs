...
fn scale_to_all_nodes(logical_function_id: &str, f: &mut crate::ir::actor::LogicalActor, available_nodes: &crate::ir::Nodes) {
    ...
    for component_instance in &f.instances {
        ...
        if let Some(active_actor) = borrowed_instance
            .try_unpack_active()
            .and_then(|active_component| active_component.as_actor())
        {
    ...
    for missing_node in missing_nodes {
        ...
        let mut node_filter = f.node_filter.clone();
        node_filter.node_ids_allowed = Some(vec![missing_node.clone()]);

        f.instances.push(std::cell::RefCell::new(
            super::super::PhysicalComponentState::request_new_instance_with_extra_constraints(node_filter),
        ));
    }
}

fn scale_singleton(logical_function_id: &str, f: &mut crate::ir::actor::LogicalActor) {
